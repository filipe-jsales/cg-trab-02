...
function Arvores(xArv, zArv){
    ...
    for (let r = raios[1]; r <= raioLimite; r+=raios[Math.floor(Math.random() * raios.length)]) {
      for (let a = 0; a <= 360; a+=angulos[Math.floor(Math.random() * angulos.length)]) {
          if (Math.random() > 0.9) {
...
var lagoZ = 30;
var lagoraio = 100;
var lago_geometry = new THREE.SphereBufferGeometry(lagoraio, 12, 6, 0, 2*Math.PI, 0, 0.5*Math.PI);
var lago_topo_geometry = new THREE.CircleGeometry(lagoraio, 12, 6);

var lagoTexture = loader.load("https://raw.githubusercontent.com/filipe-jsales/cg-trab-02/master/water-texture.jpg");
	lagoTexture.wrapS = lagoTexture.wrapT = THREE.RepeatWrapping; 
	lagoTexture.repeat.set(30, 30);
	var lago_material = new THREE.MeshPhongMaterial( { map: lagoTexture, side: THREE.BackSide } );
...
function CriarCasa(casaX, casaZ, rotação){
    ...
    // Paredes
    var geometria = new THREE.BoxGeometry(larguracubo, tamanhocubo, comprimentocubo);
    ...
    porta.position.set(cubox, cuboy - (tamanhocubo*0.4), cuboz+comprimentocubo/2);

    // Janela
    geometria = new THREE.BoxGeometry(0.1, tamanhocubo*0.5, 2.5);
    var materialjanela = new THREE.MeshLambertMaterial({map: loader.load('https://raw.githubusercontent.com/filipe-jsales/cg-trab-02/master/ywindow.jpg')});
...
for (let i = 30; i < 150; i+=20) {
    CriarCasa(circleX(raioCasa, i)+xCasas, circleY(raioCasa, i)+zCasas, (180 + i)*Math.PI/180);
}
